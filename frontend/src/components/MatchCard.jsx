...
const buildRadarData = (metricCards, match) => {
  const fallbackMetrics = match?.metrics || {};
  const fallbackScoring = match?.scoring || {};

  const scoreFromCards = (keys, fallbackValue = 0) => {
    const found = findMetricCard(metricCards, keys);
    if (found && Number.isFinite(Number(found.score))) {
      return Number(found.score);
    }
  ...
  return [
    {
      subject: '라인전',
      score: scoreFromCards(
        ['gd15', 'line', 'growth', 'lane'],
        Math.max(0, Math.min(100, 50 + safeNumber(fallbackScoring?.growthScore, 0) * 50))
      ),
    },
    {
      subject: '합류',
      score: scoreFromCards(
        ['kp', 'killParticipation', 'participation'],
        Math.max(0, Math.min(100, safeNumber(fallbackMetrics?.killParticipation, 0) * 100))
      ),
    },
    {
      subject: '오브젝트',
      score: scoreFromCards(
        ...
        Math.max(0, Math.min(100, safeNumber(fallbackMetrics?.objectiveParticipationScore, 0) * 10))
      ),
    },
    {
      subject: '시야',
      score: scoreFromCards(
        ['vision'],
        Math.max(0, Math.min(100, safeNumber(fallbackMetrics?.visionPerMinute, 0) * 30))
      ),
    },
    {
      subject: '효율',
      score: scoreFromCards(
        ['damageEff', 'efficiency', 'damageConversion', 'dpm', 'gpm'],
        Math.max(0, Math.min(100, 50 + safeNumber(fallbackScoring?.efficiencyScore, 0) * 50))
      ),
    },
    {
      subject: '데스관리',
      score: scoreFromCards(
        ['throwDeath', 'deadTime', 'survival'],
        Math.max(0, Math.min(100, 50 + safeNumber(fallbackScoring?.survivalScore, 0) * 50))
      ),
    },
  ];
};
  ...
  return [
    {
      ...
      score: Math.max(0, Math.min(100, 50 + safeNumber(scoring.growthScore, 0) * 50)),
      description: '15분 기준 골드·CS·경험치 차이를 반영합니다.',
    },
    {
      key: 'kp',
      label: '합류',
      displayValue: `${Math.round(safeNumber(metrics.killParticipation, 0) * 100)}%`,
      score: Math.max(0, Math.min(100, safeNumber(metrics.killParticipation, 0) * 100)),
    ...
    },
    {
      ...
      score: Math.max(0, Math.min(100, safeNumber(metrics.objectiveParticipationScore, 0) * 10)),
    ...
    },
    {
      ...
      score: Math.max(0, Math.min(100, safeNumber(metrics.visionPerMinute, 0) * 30)),
    ...
    },
    {
      ...
      score: Math.max(0, Math.min(100, 50 + safeNumber(scoring.efficiencyScore, 0) * 50)),
      description: '먹은 골드 대비 딜 기여 효율을 반영합니다.',
    },
    {
      key: 'throwDeath',
      label: '데스관리',
      displayValue: `패널티 ${safeNumber(metrics.throwDeathPenalty, 0)}`,
      score: Math.max(0, Math.min(100, 50 + safeNumber(scoring.survivalScore, 0) * 50)),
...
const MatchCard = ({ match, isExpanded, onToggle }) => {
  ...
  const finalDelta = safeNumber(match?.scoring?.finalDelta, null);
  const performanceScore = safeNumber(match?.scoring?.performanceScore, null);
  const perfIndex = safeNumber(match?.scoring?.perfIndex, null);

  const hasDeltaChip = Number.isFinite(finalDelta);
  const hasPerfChip = Number.isFinite(performanceScore);
      ...
      >
        ...
        <div className="flex-1 p-3 flex items-center gap-4">
          ...
          <div className="flex flex-col items-center flex-1">
            ...
            {(hasDeltaChip || hasPerfChip || hasPiChip) && (
                    ...
                    <div className="w-full h-40">
                      <ResponsiveContainer width="100%" height="100%">
                        <RadarChart cx="50%" cy="50%" outerRadius="65%" data={radarData}>
                          ...
                          <PolarAngleAxis dataKey="subject" tick={{ fill: '#A69CAC', fontSize: 10 }} />
                          <Radar
                            name="지표"
                            dataKey="score"
                            stroke="#F1DAC4"
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          ...
                          >
                            ...
                            <Legend
                              verticalAlign="top"
                              height={24}
                              wrapperStyle={{ fontSize: '10px', color: '#A69CAC' }}
                            />
                            <Line
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {comments.length > 0 ? (
                        comments.map((item, idx) => (
                          ...
                          >
                            {item.type === 'good' && (
                              <ThumbsUp className="w-3 h-3 text-emerald-300 mt-0.5 flex-shrink-0" />
                            )}
                            {item.type === 'warning' && (
                              <AlertCircle className="w-3 h-3 text-yellow-300 mt-0.5 flex-shrink-0" />
                            )}
                            {item.type === 'bad' && (
                              <AlertCircle className="w-3 h-3 text-rose-300 mt-0.5 flex-shrink-0" />
                            )}
